"""
This code is the Frontier AI paper
    ...
    def th_data_exchange(self):# thread config of read data from mocap and send packed msg to record file.
        ...
        while self.run_event.is_set() and self.th2_flag:
            try:
                ...
                pxold1 = self.l1*e01
                l1 = self.l1
                z_new1 = self.funcRK4_z_update(dt,l1,self.eta01,self.eta1,e01)
                self.d_est_old[0] = z_new1 + pxold1


                pxold2 = self.l2*e02
                l2 = self.l2 
                z_new2 = self.funcRK4_z_update(dt,l1,self.eta02,self.eta2,e02)
                self.d_est_old[1] = z_new2 + pxold2

                pxold3 = self.l3*e03
                l3 = self.l3 
                z_new3 = self.funcRK4_z_update(dt,l3,self.eta03,self.eta3,e03)
                self.d_est_old[2] = z_new3 + pxold3

                pxold4 = self.l4*e04
                l4 = self.l4 
                z_new4 = self.funcRK4_z_update(dt,l4,self.eta04,self.eta4,e04)
