function par_set=funcSimDOBx1disturb(par_set)
close all
%% Input Output data
...
tau_d(10/Ts:end,1)=1.0;
disturb=zeros(length(timeArray),1);
disturb_est=zeros(length(timeArray),1);
%%%%%% ndob
max_disturbance_est_err=5;
ndob_g2=[1;0];
ndob_lx=[1,0];
% smc_eta=(smc_lambda+ndob_lx*ndob_g2)*max_disturbance_est_err;
x2_filter=zeros(3,1);
...
for i=1:length(timeArray)-1
    ...
    disturb_est(i,1)=ndob_d_est(i,1);
    
    smc_s=de(i,1) + smc_lambda*e(i,1)+ndob_d_est(i,1);
%     max_disturbance_est_err=Km*(x1(i,1)/M);
    smc_eta=(smc_lambda+ndob_lx*ndob_g2)*max_disturbance_est_err+0.01;
    if smc_s > smc_epsilon
        smc_s_sat=smc_s/abs(smc_s);
    else
        smc_s_sat=smc_s/smc_epsilon;
    end
    u(i,1)=(-1/b_x*(f1- ddxd(i,1)+ smc_lambda*x2_filt(i,1)-smc_lambda*dxd(i,1)+...
                smc_lambda*ndob_d_est(i,1)+smc_eta*smc_s_sat));
    ...
    dx1(i,1)=x2(i,1)+disturb(i,1);
    dx2(i,1)=f1+b_x*u(i,1);
...
xlabel('time')
subplot(4,1,4)
plot(timeArray(1:end),disturb_est(1:end),'b','LineWidth',2)
hold on
plot(timeArray(1:end),disturb(1:end),'r')
